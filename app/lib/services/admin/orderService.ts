...
/**
 * Cancel an order or specific items in an order with optional partial quantity cancellation (Admin)
 ...
 * @param cancellations - Optional array of {itemId, quantityToCancel} for partial quantity cancellation
 * @returns Promise<boolean> - true if successful, false otherwise
 */
export const cancelOrder = async (
  ...
  try {
    ...
    if (hasCancellations) {
      // Send partial quantity cancellations
      payload = { cancellations };
    } else if (itemIds && itemIds.length > 0) {
      // Send full item cancellations (legacy)
      payload = { itemIds };
    }
    ...
    const response = await axiosInstance.put<{
      ...
      message: string;
      refundAmount?: number;
    }>(`${BASE_URL}/cancel-order/${orderId}`, payload);

    closeLoading();

    if (response.data.success) {
      const refundMsg = response.data.refundAmount
        ? ` Refund amount: ₹${response.data.refundAmount}`
        : "";
      showSuccessMessage(
        response.data.message ||
          (hasCancellations || itemIds
            ? `Items cancelled successfully.${refundMsg}`
            : `Order cancelled successfully.${refundMsg}`)
      );
      return true;
    } else {
      showErrorMessage(
    ...
    if (axiosError.response) {
      const status = axiosError.response.status;
      const errorData = axiosError.response.data;
      let errorMessage = "Failed to cancel items";

      switch (status) {
        case 400:
          errorMessage =
            errorData?.message || "Cannot cancel these items";
          break;
        case 404:
          errorMessage =
            "Cancel order endpoint not found. Please contact support.";
          break;
        case 401:
          errorMessage = "Access denied. Admin authentication required.";
          break;
        default:
          errorMessage =
            errorData?.message || "Failed to cancel items";
      }

      showErrorMessage(errorMessage);
    } else {
      showErrorMessage("Failed to cancel items. Please try again later.");
    }
