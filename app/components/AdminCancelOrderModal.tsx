...
interface AdminCancelOrderModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (cancellations: Array<{ itemId: string; quantityToCancel: number }>, refundAmount: number) => void;
  orderId: string;
...
export default function AdminCancelOrderModal({
  ...
  const [selectedItems, setSelectedItems] = useState<Map<string, { checked: boolean; qty: number }>>(new Map());
  ...
  useEffect(() => {
    if (isOpen && autoCheckProductTitles.length > 0) {
      const itemsToCheck = new Map<string, { checked: boolean; qty: number }>();
      orderItems.forEach((item) => {
        const shouldCheck = autoCheckProductTitles.some(
          (title) =>
            item.Product?.title?.toLowerCase() === title.toLowerCase()
        );
        itemsToCheck.set(item.id, {
          checked: shouldCheck,
          qty: shouldCheck ? item.quantityRequested : 0,
        });
      });
      setSelectedItems(itemsToCheck);
    } else if (isOpen) {
      // Initialize all items with qty 0
      const itemsMap = new Map<string, { checked: boolean; qty: number }>();
      orderItems.forEach((item) => {
        itemsMap.set(item.id, { checked: false, qty: 0 });
      });
  ...
  useEffect(() => {
    ...
    const totalQtyCancelled = Array.from(selectedItems.values()).reduce((sum, item) => sum + item.qty, 0);
  ...
  const handleItemToggle = (itemId: string) => {
    ...
    const current = newMap.get(itemId) || { checked: false, qty: 0 };
    const item = orderItems.find((i) => i.id === itemId);
    
    newMap.set(itemId, {
      checked: !current.checked,
      qty: !current.checked ? item?.quantityRequested || 0 : 0,
    });
  ...
  const handleQtyChange = (itemId: string, qty: number) => {
    const newMap = new Map(selectedItems);
    const item = orderItems.find((i) => i.id === itemId);
    const maxQty = item?.quantityRequested || 0;
  ...
  const selectedItemsTotal = Array.from(selectedItems.entries()).reduce(
    (sum, [itemId, { qty }]) => {
      const item = orderItems.find((i) => i.id === itemId);
      if (item && qty > 0) {
  ...
  const selectedItemCount = Array.from(selectedItems.values()).filter((item) => item.qty > 0).length;

  const handleConfirm = () => {
    const cancellations = Array.from(selectedItems.entries())
      .filter(([_, { qty }]) => qty > 0)
      .map(([itemId, { qty }]) => ({
        itemId,
    ...
    if (cancellations.length > 0) {
      onConfirm(cancellations, finalRefundAmount);
    }
  };
          ...
          <div className="space-y-3 mb-6 max-h-64 overflow-y-auto border border-gray-200 rounded-lg p-4">
            {orderItems.filter((item) => item.status !== "canceled").length > 0 ? (
              orderItems.filter((item) => item.status !== "canceled").map((item) => {
                ...
                const itemState = selectedItems.get(item.id) || { checked: false, qty: 0 };
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      {/* Quantity Input for Partial Cancellation */}
                      {itemState.checked && (
                        <div className="mt-2 flex items-center gap-2">
                          ...
                          <span className="text-xs text-gray-600">
                            / {item.quantityRequested} = ₹{(parseFloat(item.unitPrice) * itemState.qty).toFixed(2)}
                          </span>
                        </div>
                      )}
                    </div>
