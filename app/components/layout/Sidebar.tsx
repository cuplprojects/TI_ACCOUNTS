...
import { UserRole, cn } from "@/app/lib/utils";
import { useAuth } from "@/app/providers/AuthProvider";
import sidebarConfig from "@/app/data/sidebar.json";
...
export default function Sidebar({ className, onNavClick }: SidebarProps) {
  ...
  const navItems = sidebarConfig.navigation.filter((item) =>
    item.userTypes.includes(userRole)
  );
      ...
      <nav className="flex-1 py-2 px-2 xs:px-3">
        <ul className="space-y-1">
          ...
          {navItems.map((item, index) => {
            const basePath = userRole === "admin" ? "" : "/seller";
